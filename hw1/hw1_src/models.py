...
class RNNattELMo(nn.Module):
    def __init__(self, input_size=300, classes=1, hidden_size=256, rec_len=112, rep_len=16, window_size=128, drop_p=0.3, num_of_words=80000):
        super(RNNattELMo, self).__init__()
        ...
        self.lstm1_rec = nn.LSTM(3072, self.hidden_size, batch_first=True, bidirectional=True)
    ...
    def forward(self, input_data_rec, input_data_rep, elmo):
        rec_elmo = []
        for sent in input_data_rec:
            out = [ self._pad_and_cut(seq=[min(ord(c), 259) for c in word],
                                      max_leng=self.max_word_len,
                                      pad=self.char_pad,
                                      tensor=True
                                      ) if word != "<pad>" else self.word_pad for word in sent ]
            rec_elmo.append([self.sos]+out+[self.eos])

        rec_elmo = torch.tensor(rec_elmo, dtype=torch.int64)
        rec_elmo = rec_elmo.to(device)
        embedding_rec = elmo.get_contexulize_embedding(rec_elmo)

        rep_elmo = []
        for sent in input_data_rep:
            out = [ self._pad_and_cut(seq=[min(ord(c), 259) for c in word],
                                      max_leng=self.max_word_len,
                                      pad=self.char_pad,
                                      tensor=True
                                      ) if word != "<pad>" else self.word_pad for word in sent ]
            rep_elmo.append([self.sos]+out+[self.eos])

        rep_elmo = torch.tensor(rep_elmo, dtype=torch.int64)
        rep_elmo = rep_elmo.to(device)
        embedding_rep = elmo.get_contexulize_embedding(rep_elmo)

        batch_size = embedding_rec.shape[0]

        embedding_rec, embedding_rep = torch.tensor(embedding_rec).to(device), torch.tensor(embedding_rep).to(device)
        x_rec = self.dropout(embedding_rec)
        x_rep = self.dropout(embedding_rep)
